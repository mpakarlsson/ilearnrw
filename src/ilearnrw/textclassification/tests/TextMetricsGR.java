...
public class TextMetricsGR extends JFrame implements TextMetricsGUI {
	private static final long serialVersionUID = 1L;
	private LanguageCode lc = LanguageCode.GR;
	...
	public static void main(String[] args) {
		try {
			...
			String databaseFile = "greek_users";// Program.getStringArg("--db",
												// args);
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					TextMetricsGR frame = new TextMetricsGR();
	...
	public TextMetricsGR() {
		...
		setTitle("iLearnRW Text Classification Tool - Greek Version");
		setSize(800, 450);
		...
		explorerPanel = new FilesExplorerPanel(user.getProfile(), tabbedPane,
				textPanel, this, greekAnalyzer);
		tabbedPane.addTab("File Explorer", explorerPanel);
		tabbedPane.addTab("User Profile", userProblemsPanel);

		// TODO change the 2 following rows / send them inside the
		// UserSeveritiesHeatMapPanel class
	...
	private void initUsers() {
		...
		try {
			...
			for (User u : mUserStore.getAllUsers()) {
				...
				ProblemDefinitionIndex problems = u.getProfile()
						.getUserProblems().getProblems();
				UserSeverities userSeverities = u.getProfile()
						.getUserProblems().getUserSeverities();
				for (int i = 0; i < problems.getProblems().length; i++) {
					int wi = 2 * problems.getRowLength(i) / (2*cnt+1) + rand.nextInt(1);
		...
		userCombobox.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				UserListBoxWrapper selectedUser = (UserListBoxWrapper) userCombobox
						.getSelectedItem();
				...
				try {
					/*
					 * Change user to reflect the new user. Note, we get the
					 * user fresh from the UserStore and do not reuse the one
					 * stored in the ComboBox.
					 */
					for (User u : mUserStore.getAllUsers())
					...
					wordPanel.reset(user.getProfile());
					userProblemsPanel.setUser(user);
					explorerPanel.setLanguageAnalyzer(greekAnalyzer);
	...
	private String[][] testTextMetrics() {
		 ...
		 * 
		 * Object tmp[] = hs.keySet().toArray(); Word w = (Word)tmp[0];
		 * System.out.println(w.toString()+ " -- "+w.getLanguageCode());
		 * WordVsProblems wp = new WordVsProblems(w.getLanguageCode());
		 * wp.insertWord(w); System.out.println(wp.toString());
		 */
		String[][] data = {
				{ "# Paragraphs", "" + tcr.getNumberOfParagraphs(),
						"Longest Word Length:",
						"" + tcr.getLongestWordLength(), "Flesch:",
						String.format("%.2f", tcr.getFlesch()) },
				{ "# Sentences:", "" + tcr.getNumberOfSentences(),
						"Longest Sentence Length:",
						"" + tcr.getLongestSentenceLength(), "Flesch-Kincaid:",
						String.format("%.2f", tcr.getFleschKincaid()) },
				{
						"# Words:",
						"" + tcr.getNumberOfTotalWords(),
						"Avg Words per Sentence:",
						String.format("%.2f", tcr.getAverageWordsPerSentence()),
						"Automated:", String.format("%.2f", tcr.getAutomated()) },
				{
						"# Distinct Words",
						"" + tcr.getNumberOfDistinctWords(),
						"Avg Syllables per Word:",
						String.format("%.2f", tcr.getAverageSyllablesPerWord()),
						"Coleman-Liau:",
						String.format("%.2f", tcr.getColemanLiau()) },
				{ "# Syllables:", "" + tcr.getNumberOfSyllables(),
						"Avg Word Length:",
						String.format("%.2f", tcr.getAverageWordLength()),
						"SMOG:", String.format("%.2f", tcr.getSmog()) },
				{
						"# Big Sentences:",
						"" + tcr.getNumberOfBigSentences(),
						"Avg Longest Word Length:",
						String.format("%.2f", tcr.getAverageLongestWordLength()),
						"Gunning FOG:",
						String.format("%.2f", tcr.getGunningFog()) },
				{ "# Polysyllabic Words:",
						"" + tcr.getNumberOfPolysyllabicWords(),
						"Avg Word Rank:",
						String.format("%.2f", tcr.getAverageWordRank()), "" },
				{ "# Letters and Numbers:",
						"" + tcr.getNumberOfLettersAndNumbers(), "", "",
	...
	private String[][] testWordMetrics() {
		String[][] data = {
				...
				{
						"Word Language",
						findLanguage(wordPanel.getText()) == LanguageCode.GR ? "Greek"
								: "English" },
				{
						"User Compatible",
						findLanguage(wordPanel.getText()) == user.getProfile()
								.getLanguage() ? "True" : "False" },
	...
	public void runTextClassifier(boolean isWord) {
		if (isWord) {
			Word ww = new GreekWord(wordPanel.getText());
			upw = new UserProblemsToWord(user.getProfile(), ww, greekAnalyzer);
			wcr = upw.getWordClassificationResults();
			wordPanel.getSmallHeatMapPanel().setClassifier(wcr);
		} else {
			upt = new UserProblemsToText(user.getProfile(), new Text(
					textPanel.getText(), lc), greekAnalyzer);
	...
	private LanguageCode findLanguage(String text) {
		char[] lowerCase = { 'α', 'β', 'γ', 'δ', 'ε', 'ζ', 'η', 'θ', 'ι', 'κ',
				'λ', 'μ', 'ν', 'ξ', 'ο', 'π', 'ρ', 'σ', 'τ', 'υ', 'φ', 'χ',
				'ψ', 'ω', 'ά', 'έ', 'ή', 'ί', 'ό', 'ύ', 'ώ', 'ϊ', 'ϋ', 'ΐ',
				'ΰ', 'ς' };
		char[] upperCase = { 'Α', 'Β', 'Γ', 'Δ', 'Ε', 'Ζ', 'Η', 'Θ', 'Ι', 'Κ',
				'Λ', 'Μ', 'Ν', 'Ξ', 'Ο', 'Π', 'Ρ', 'Σ', 'Τ', 'Υ', 'Φ', 'Χ',
				'Ψ', 'Ω', 'Ά', 'Έ', 'Ή', 'Ί', 'Ό', 'Ύ', 'Ώ', 'Ϊ', 'Ϋ' };
	...
	private int getColorMode() {
		String t = "";
		try {
			t = new Scanner(new File("params/colors.txt")).useDelimiter("\\A")
					.next();
